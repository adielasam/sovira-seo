...
// Replace runSequence
const oldSequenceStart = 'function runSequence() {';
const oldSequenceEnd = 'runSequence()\\n    return'; // this might not match exactly due to whitespace, so I'll just use split

let parts = code.split('function runSequence() {');
if (parts.length < 2) throw new Error("Could not find function runSequence() {");
let afterSequence = parts[1].split('runSequence()\\n    return');

// actually, let's just use replace with a safer regex that matches everything between "function runSequence() {" and "runSequence()\n    return"
const sequenceRegex = /function runSequence\(\) \{[\s\S]*?runSequence\(\)\r?\n\s*return/;

const newSequence = `function runSequence() {
...
if (!sequenceRegex.test(code)) throw new Error("Could not match sequenceRegex");
code = code.replace(sequenceRegex, newSequence);

// Replace COMPETITORS VIEW with ANALYSER VIEW
const viewRegex = /\{\/\* --- COMPETITORS VIEW ---\s*\*\/\}[\s\S]*?(?=\{\/\* --- CONTENT AI VIEW ---\s*\*\/\})/;
...
if (!viewRegex.test(code)) throw new Error("Could not match viewRegex");
code = code.replace(viewRegex, analyserView);
