...
export async function POST(req: Request) {
  try {
    ...
    const fetchHeaders = {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 SoviraAuditBot/1.0',
    ...
    // 2. Technical Checks
    const robotsRes = await fetch(`${baseUrl}/robots.txt`, { headers: fetchHeaders }).catch(() => null)
    const robotsTxt = robotsRes?.ok ? await robotsRes.text() : ''
    
    const soft404Res = await fetch(`${baseUrl}/sovira-404-test-${Date.now()}`, { headers: fetchHeaders }).catch(() => null)
    const isSoft404 = soft404Res ? soft404Res.status !== 404 : false

    const llmsRes = await fetch(`${baseUrl}/llms.txt`, { headers: fetchHeaders }).catch(() => null)
    const agentsRes = await fetch(`${baseUrl}/agents.txt`, { headers: fetchHeaders }).catch(() => null)
